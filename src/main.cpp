...
#if defined(NDEBUG)
# error "Krown cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2a25065d6008262a30c5b8e5e648acd4d6964033941c0507c0928f927b544ada");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Krown: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Krown Signed Message:\n";
...
{
    // Krown: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 20 * COIN; // 20 coins as reward

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 10000); // Krown: will half every 10000 blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Krown: 1 day
static const int64 nTargetSpacing = 5 * 60; // Krown: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Krown: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Krown: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe517262cd786998606e06e672e558b6dd317e9ec114c860bbd3b56d897d6795e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 04/feb/2018 Rare glimpse of super blue blood moon";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0425ea6af874f2da975e088bcf293da78263fbc4d77c30886a339218dd63e66c2fc189703a9fae7f5a0e4917e7ca039c4a0f4fcc1b2980ca81f11ad0b6030f063c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517722598;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086001929;

        if (fTestNet)
        {
            block.nTime    = 1517722554;
            block.nNonce   = 385653114;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0eb40be15595683b744f11ff91000448086560aa1a4a6b3eaf3020e3165a7ea4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb5, 0xda }; 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KrownMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KrownMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KrownMiner(CWallet *pwallet)
{
    printf("KrownMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("krown-miner");
    ...
    try { loop {
        ...
        printf("Running KrownMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KrownMiner, pwallet));
}
